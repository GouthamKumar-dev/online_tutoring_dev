...
const CoursesPage: React.FC = () => {
  ...
  // Fetch courses when applied filters change
  useEffect(() => {
    const filters: any = {};
    ...
    // Only fetch if we have some filters or if it's the initial load
    if (Object.keys(filters).length > 0 || categories.length > 0) {
      dispatch(fetchCourses(filters));
    }
  }, [dispatch, appliedFilters, categories]);
                  ...
                  >
                    <div className="h-48 bg-gray-200 flex items-center justify-center">
                      {course.pdfPath1 ? (
                        <div className="flex flex-col items-center justify-center text-gray-600">
                          ...
                          <span className="text-sm">Course Material</span>
                        </div>
                      ) : (
                    ...
                    <div className="p-6">
                      ...
                      <div className="text-sm text-gray-500 mb-4">
                        <p>Tutor: {course.tutor || "TBA"}</p>
                        <p>Duration: {course.duration || "TBA"}</p>
                        <p>Level: {course.level || "TBA"}</p>
                      </div>

                      <div className="flex items-center justify-between">
                        <span
                          className={`text-lg font-bold ${COLOR_CLASSES.textPrimary}`}
                        >
                          {course.price || "Price TBA"}
                        </span>
                        <button
